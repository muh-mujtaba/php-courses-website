...
<div class="container-fluid">
  ...
  if(isset($_GET['page'])){
   ...
   $offset = ($page - 1) * $limit;
   $admin ="admin";
  $sql = "select * from signup where signup.status like 'a%' order by signup.user_id desc limit {$offset},{$limit} ";
  $result = mysqli_query($conn, $sql);
  ...
  ?>
    ...
    <?php
$sql1 = "select * from signup where signup.status like 'a%'";
...
if($page>1){
  echo '<li><a href="dashadmin.php?page='.($page - 1).'">Prev</a></li>';
}
for($i = 1; $i<=$total_page; $i++){
  ...
  echo '<li><a class="'.$active.'" href="dashadmin.php?page='.$i.'">'.$i.'</a></li>';
}
if($total_page > $page){
  echo '<li><a href="dashadmin.php?page='.($page + 1).'">Next</a></li>';
}
echo " </ul>";
?>
</div>
